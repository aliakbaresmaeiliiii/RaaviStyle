...
│   └── web/                      # Empty Next.js 16 storefront (@raavistyle/web)
├── eslint.config.ts              # Root ESLint: @medusajs/eslint-plugin recommended
├── turbo.json                    # Task graph: build, dev, start, lint, test, seed
```

The storefront lives in `apps/web` (not `apps/storefront`). It is an empty Next.js App Router app. Do not replace it with the Medusa Next.js Starter unless the user asks.
...
```bash
<pm> run dev                # all apps
<pm> run dev:backend        # backend only (http://localhost:9000, admin at /app)
<pm> run dev:web            # storefront only (http://localhost:3000)
```
...
cd apps/web && <pm> run lint           # next lint
```
...
- `building-storefronts` — anything under `apps/web`
...
- Running storefront commands against `apps/storefront/` — this repo uses `apps/web`.
